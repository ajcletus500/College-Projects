...
single_thread_workloads = ['crafty', 'bzip2_source']

#GLOBALS
bench_dir = "/mnt/c/Users/hp/OneDrive/GMU_Notes/Sem4/611/Assignment1/SMTSIM/benchmarks"
scr_dir = "/mnt/c/Users/hp/OneDrive/GMU_Notes/Sem4/611/Assignment2/Shells_256/" #--Please provide the exact path
exe = "smtsim"
exe_path = os.path.join("/mnt/c/Users/hp/OneDrive/GMU_Notes/Sem4/611/Assignment1/SMTSIM/smtsim/build.linux-amd64", exe)
sims_group = "/mnt/c/Users/hp/OneDrive/GMU_Notes/Sem4/611/Assignment1/SMTSIM/results/HW2/IPC_256/"  #-Please provide the exact path
thread_length = 1e6
ff_dist = 1e6
host = ''
blue =[1, 2, 4]
...
for b in range (b_len):
	...
	for a in range (a_len):
		new_amber = [amber[a],amber[a]]
		for w in range (w_len):
			new_white = [white[w]]
			for o in range (0,4,2):
				new_olive = [olive[o],olive[o+1],olive[o],olive[o+1]]
				o=o+1
				#print(new_olive)
				#print(o)
				for y in range (y_len):
					new_yellow = [yellow[y],yellow[y],yellow[y]]
					#for g in range (g_len):
						#new_green = [green[g],green[g]]
						
					c0= new_yellow + new_amber + new_white + new_olive + miss + new_blue
					p=p+1
					if c0==yellow16 or c0==yellow32 or c0==yellow48 or c0==amber32 or c0==amber64 or c0==amber96 or c0==white32 or c0==white64 or c0==white96 or c0==olive16 or c0==olive32 or c0==blue1 or c0==blue2 or c0==blue4:
						with open('out_graph_256.csv', 'a') as out_file:
							out_file.write("%s = %s \n" %(c0,p) )
					with open('out_file_256.csv', 'a') as out_file:
						out_file.write("%s = %s \n" %(c0,p) )
					print(c0)
...
#Creates a list of dictionaries. Each dictionary has a key for each config parameter
					def create_configurations():
						...
						for c in configs_to_test:    
							#print c
							conf_dict = {}
							conf_dict['iqs']=c[0]  # Integer Instruction queue size 	yellow
							conf_dict['fqs']=c[1]  # Fp Instruction queue size  		yellow
							conf_dict['lsq']=c[2]  # Load Store Queue size 				yellow
							conf_dict['ipr']=c[3]  # Integer phys. regs 				amber
							conf_dict['fpr']=c[4]  # Fp phys. regs  					amber
